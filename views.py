...
class PaymentMailView(TestContributieAdmin, View):
    ...
    def post(self, request, pk):
        ...
        if payment.method.description == "Incasso":
            ...
            if payment.status.description == 'Gepland':
                template_name = 'jarrbo_contributie/aankondiging_incasso.txt'
                template = get_template(template_name)
                ctx = {'payment': payment,}
                subject = 'Aankondiging incasso contributie'
...
def NotPayedExport(request):
    ...
    writer = csv.writer(response, delimiter=";")
    # Write a first row with header information
    writer.writerow(['relatiecode', 'naam', 'roepnaam', 'achternaam', \
                     'leeftijdscategorie', 'betaalwijze', 'contributie', 'betaald'])
    ...
    for c in Contribution.seizoen_objects.all():
        ...
        if c.total_contribution > betaald:
            writer.writerow([c.member.relatiecode, c.member.fullname, c.member.roepnaam, c.member.achternaam, \
                              c.member.lc, c.payment_method, c.tc, betaald])
    return response
